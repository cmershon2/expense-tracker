...
public class CategoryRepositoryImpl  implements CategoryRepository{
    ...
    private static final String SQL_FIND_ALL = "SELECT C.CATEGORY_ID, C.USER_ID, C.TITLE, C.ICON, C.DESCRIPTION, " +
            "COALESCE(SUM(T.AMOUNT), 0) TOTAL_EXPENSE " +
            "FROM ET_TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID = T.CATEGORY_ID " +
            "WHERE C.USER_ID = ? GROUP BY C.CATEGORY_ID";
    private static final String SQL_FIND_BY_ID = "SELECT C.CATEGORY_ID, C.USER_ID, C.TITLE, C.ICON, C.DESCRIPTION, " +
            "COALESCE(SUM(T.AMOUNT), 0) TOTAL_EXPENSE " +
            "FROM ET_TRANSACTIONS T RIGHT OUTER JOIN ET_CATEGORIES C ON C.CATEGORY_ID = T.CATEGORY_ID " +
            "WHERE C.USER_ID = ? AND C.CATEGORY_ID = ? GROUP BY C.CATEGORY_ID";
    private static final String SQL_CREATE = "INSERT INTO ET_CATEGORIES (CATEGORY_ID, USER_ID, TITLE, DESCRIPTION, ICON) VALUES(NEXTVAL('ET_CATEGORIES_SEQ'), ?, ?, ?, ?)";
    private static final String SQL_UPDATE = "UPDATE ET_CATEGORIES SET TITLE = ?, ICON = ?, DESCRIPTION = ? " +
    ...
    @Override
    public Integer create(Integer userId, String title, String description, String icon) throws EtBadRequestException {
        try {
            ...
            throw new EtBadRequestException("Invalid request: "+e+" DONE!");
        }
    }
    ...
    public void update(Integer userId, Integer categoryId, Category category) throws EtBadRequestException {  
        try{
            jdbcTemplate.update(SQL_UPDATE, new Object[]{category.getTitle(), category.getIcon(), category.getDescription(), userId, categoryId});
        } catch (Exception e) {
